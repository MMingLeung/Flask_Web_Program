...
@main.route('/follower-by/<username>')
...
def moderate():
    page = request.args.get('page', 1, type=int)#渲染的页数从请求的查询字符串（request.args）中获取，参数type=int保证参数无法转成整数时，返回默认值。
    ...
    #paginate(必需参数， per_page=制定每页显示的页面， error_out=True时，如果请求超出范围，返回404错误，False时，返回空列表)
    comments = pagination.items
